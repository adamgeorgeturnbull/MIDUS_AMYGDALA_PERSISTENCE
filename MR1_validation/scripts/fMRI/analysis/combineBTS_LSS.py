...
Note: despite the M2ID in the filename (kept for naming consistency with M3),
the ID column written is MIDUSID (the MR1 participant identifier).
"""

import pandas as pd
from pathlib import Path

input_dir = Path("/scratch/groups/fvlin/MIDUS/MR1_reproduction_20260828/BetaSeries_LSS_output")
...
for sub_dir in sorted(input_dir.iterdir()):
    ...
    if 'subid' in df.columns:
        df['MIDUSID'] = df['subid'].str.replace('^sub-', '', regex=True).astype(int)
