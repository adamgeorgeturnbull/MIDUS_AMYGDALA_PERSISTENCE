...
#SBATCH --output=/scratch/groups/fvlin/MIDUS/MR1/log/betaSeries_LSS_%A_%a.log
#SBATCH --error=/scratch/groups/fvlin/MIDUS/MR1/log/betaSeries_LSS_%A_%a.err
#SBATCH --time=24:00:00
#SBATCH --cpus-per-task=4
#SBATCH --mem-per-cpu=6G
#SBATCH --mail-user=aturnbu2@stanford.edu
#SBATCH --mail-type=ALL
#SBATCH --array=1-127%20
...
out_dir=/scratch/groups/fvlin/MIDUS/MR1/BetaSeries_LSS_output
mkdir -p $out_dir
...
for run in runs:
    ...
    # MR1 events use full words; normalise to abbreviations
    val_map = {'negative': 'neg', 'neutral': 'neu', 'positive': 'pos'}
    events['valence'] = events['valence'].map(val_map).fillna(events['valence'])
    # MR1 has no onset_trimmed — subtract dummy volumes from onset
    N_DUMMIES = 4
