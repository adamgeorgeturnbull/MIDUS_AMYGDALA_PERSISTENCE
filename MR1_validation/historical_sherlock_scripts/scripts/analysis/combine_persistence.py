...
This script renames `subject` -> `M2ID` (stripping the BIDS `sub-` prefix) and
writes the two files the merge step reads:
    negative_persistence_cross_run.csv
    positive_persistence_cross_run.csv
with columns: M2ID, hemisphere, mean_r, median_r, std_r, n_pairs

NOTE on IDs: the `sub-XXXX` label is the Refresher participant ID. We keep the
column named `M2ID` so the existing merge/analysis code runs unchanged, but this
is the *Refresher* subject identifier — it must line up with whatever ID the
Refresher behavioral data uses. Revisit once the behavioral ID scheme is known
(see 01_harmonize_ids.py).
...
SUMMARY_DIR = Path(os.environ.get(
    "MR1_SUMMARY_DIR",
    "/scratch/groups/fvlin/MIDUS/MR1/voxelwise_betas_summary",
))
...
KEEP_COLS = ["M2ID", "hemisphere", "mean_r", "median_r", "std_r", "n_pairs"]
...
for src_name, out_name in PAIRS:
    ...
    if not src.exists():
        ...
        continue
    df = pd.read_csv(src)
    df = df.rename(columns={"subject": "M2ID"})
    df["M2ID"] = df["M2ID"].astype(str).str.replace("^sub-", "", regex=True)
    df = df[[c for c in KEEP_COLS if c in df.columns]]
    out = OUT_DIR / out_name
    df.to_csv(out, index=False)
    print(f"Wrote {out}  ({df['M2ID'].nunique()} subjects, {len(df)} rows)")
