#!/usr/bin/env python3
"""
figure4_fc_affect.py

Figure 4 for the MIDUS Amygdala Persistence manuscript: the M3 association
between left-amygdala-anterior-vmPFC functional connectivity and daily affect.

    Panel A  Positive affect
    Panel B  Negative affect

Both outcomes are shown for transparency. Log-transformed negative affect and
posterior vmPFC are deliberately excluded from this figure. Neither panel is
labelled "significant" or "nonsignificant" - the annotated p-values speak for
themselves. The anterior and posterior vmPFC targets are co-equal in this
study; no text in this figure or in this module ranks one above the other.

Predictor: l_amyg-ant_vmPFC_neg_vs_neu - left-amygdala-anterior-vmPFC
functional connectivity, negative-minus-neutral contrast, Fisher z.
...
scripts/analysis/04_fc_affect.py uses, so the sample, covariate set, family
grouping, and model are identical by construction:
...
Note that 04_fc_affect.py does NOT call prepare_persistence_vars: the
beta-series FC columns are already Fisher-z transformed, so no transformation
step is applied here either. This is the one structural difference from
figure3_persistence_affect.py's sample construction.
...
Two behaviours inherited from run_mlm matter for interpretation:
...
The predictor name contains a hyphen, which Patsy cannot parse. run_mlm
sanitizes it to l_amyg_ant_vmPFC_neg_vs_neu and copies the column; the refit
here performs the identical substitution.
...
Inference is one-tailed with prespecified directions: positive for PA_score,
negative for NA_score.

Validation
----------
Three independent checks must all pass before anything is drawn:

  1. results/tables/04_fc_affect/mlm.csv is read and its PA_score and NA_score
     rows for this predictor are compared against the frozen expected values
     below, including the recorded optimizer.
...
Annotation placement is configurable per panel via ANNOT_POS, so it can be
retuned after inspecting the authoritative render without touching any model
or validation code. Each panel is positioned independently on whatever space
that panel has, and is not held consistent with the other panel or with the
other figures. Both currently sit in the top right.
...
Outputs
-------
    results/figures/figure4_fc_affect.svg
    results/figures/figure4_fc_affect.tiff   (300 dpi, LZW)
...
# Configuration
# ============================================================================
MLM_CSV = RESULTS_DIR / "04_fc_affect" / "mlm.csv"
OUT_DIR = Path("results/figures")
OUT_STEM = "figure4_fc_affect"
FIG_DPI = 300

PREDICTOR = "l_amyg-ant_vmPFC_neg_vs_neu"
...
X_LABEL = (
    "Left-amygdala–anterior-vmPFC connectivity\n"
    "(negative − neutral, Fisher z)"
)

# Annotation anchor per outcome, in axes fraction: (x, y, ha, va).
# Each panel is set independently and may sit wherever that panel has room;
# placement is deliberately NOT held consistent with the other panel or with
# the other figures. Retune here after inspecting a render - no model or
# validation code depends on these values.
ANNOT_POS = {
    "PA_score": (0.965, 0.975, "right", "top"),
    "NA_score": (0.965, 0.975, "right", "top"),
}
...
EXPECTED_MLM = {
    "PA_score": {
        "n": 81,
        "beta": -0.017756501529098422,
        "se": 0.22192881773733486,
        "z": -0.080009895560811,
        "p": 0.5318853071573921,
        "optimizer": "lbfgs",
    },
    "NA_score": {
        "n": 81,
        "beta": -0.10554044412607674,
        "se": 0.06007889493914433,
        "z": -1.7566974930710983,
        "p": 0.039484692539362376,
        "optimizer": "lbfgs",
    },
}
...
# ============================================================================
# Appearance — vmPFC green, consistent with Figure 1's vmPFC colour.
# Dimensions, typography, margins, point size and spines match Figure 3.
# ============================================================================
FC_FILL = "#4CAF35"  # vmPFC green — point faces
FC_LINE = "#2A6B1A"  # dark vmPFC green — point edges and fitted line
INK = "#1A1A1A"
INK_SOFT = "#5A5A5A"
...
def load_stored_mlm():
    ...
    if not MLM_CSV.exists():
        _fail(
            ...
            "  Run scripts/analysis/04_fc_affect.py first, or run this\n"
...
def build_sample():
    ...
    try:
        ...
        _fail(
            ...
            "  project, where data/processed/midus_with_fmri.csv and the\n"
            "  beta-series FC file both exist."
        )

    # No prepare_persistence_vars call: 04_fc_affect.py does not use it, since
    # the beta-series FC columns are already Fisher-z transformed.
    if PREDICTOR not in df.columns:
        _fail(
            f"predictor '{PREDICTOR}' is absent after load_master(fc=True); "
            "the beta-series FC file did not supply the anterior-vmPFC "
            "negative-minus-neutral connectivity column."
        )
...
# Drawing
# ============================================================================
def draw_panel(ax, letter, title, y_label, x, y_adj, beta, y_mean, annot, annot_pos):
    ax.scatter(
        x,
        y_adj,
        s=13,
...
def build_figure(panel_data):
    ...
    # bottom is 0.250 rather than Figure 3's 0.215 because this x label runs
    # to two lines; every other margin matches Figure 3 exactly.
    fig.subplots_adjust(left=0.085, right=0.985, top=0.905, bottom=0.250, wspace=0.28)
    return fig
...
def main():
    print("=" * 72)
    print("Figure 4 — anterior-vmPFC connectivity → daily affect (M3)")
    print("=" * 72)
    ...
    save_figure(fig)
    plt.close(fig)
    print("\nDone. Figure 4 is fully assembled; no manual composition required.")
    return 0
