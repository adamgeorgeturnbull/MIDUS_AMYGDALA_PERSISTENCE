...
#SBATCH --output=/scratch/groups/fvlin/MIDUS/log/reconall_%A_%a.log
#SBATCH --error=/scratch/groups/fvlin/MIDUS/log/reconall_%A_%a.err
...
export SUBJECTS_DIR=/scratch/groups/fvlin/MIDUS/freesurfer_subjects

# Get the subject ID for this task
SUBJ=$(sed -n "${SLURM_ARRAY_TASK_ID}p" /scratch/groups/fvlin/MIDUS/recon_error_list.txt)

# Path to N4-corrected T1w image
T1_IMAGE=/scratch/groups/fvlin/MIDUS/M3/M3_ImagingSession/${SUBJ}/anat/${SUBJ}_T1w_N4Corrected.nii.gz
