...
#
# Takes subject-level connectivity maps from seedbasedBStaskFC.sh (v2, 3-cond
# GLM) and runs second-level (group) analyses using TFCE-based permutation
# testing (Smith & Nichols, 2009) via nilearn's non_parametric_inference.
#
# For each map type (neg, neu, pos, neg_vs_neu, neg_vs_pos) x seed:
#   1. Group mean: One-sample t-test on connectivity maps
#   2. Covariate analysis: Tests whether left amygdala persistence (mean_z)
#      predicts voxelwise connectivity
#
...
#
# Output per seed per map type:
#   - group_mean_<seed>_<maptype>_tstat.nii.gz
#   - group_mean_<seed>_<maptype>_logp_max_tfce.nii.gz
#   - group_mean_<seed>_<maptype>.png
#   - group_covariate_<seed>_<maptype>_tstat.nii.gz
#   - group_covariate_<seed>_<maptype>_logp_max_tfce.nii.gz
#   - group_covariate_<seed>_<maptype>.png
#
...
# Map types to analyze: per-condition + contrasts
MAP_TYPES = ['neg', 'neu', 'pos', 'neg_vs_neu', 'neg_vs_pos']
...
# Build maps dict: maps[seed][map_type] = list of file paths
# Also track which subjects have each map type
maps = {seed: {mt: [] for mt in MAP_TYPES} for seed in seeds}
subjects_with_maps = {seed: {mt: [] for mt in MAP_TYPES} for seed in seeds}

for sub in all_subjects:
    for seed in seeds:
        for mt in MAP_TYPES:
            map_file = output_dir / sub / f"{sub}_{seed}_seedFC_{mt}.nii.gz"
            if map_file.exists():
                maps[seed][mt].append(str(map_file))
                subjects_with_maps[seed][mt].append(sub)

# Print summary of available maps
for seed in seeds:
    print(f"\n{seed}:")
    for mt in MAP_TYPES:
        print(f"  {mt}: {len(maps[seed][mt])} subjects")

# ------------------------
# Group-level analysis per seed x map type
# ------------------------
for seed in seeds:
    for mt in MAP_TYPES:
        n_subj = len(maps[seed][mt])
        print(f"\n{'=' * 60}")
        print(f"Processing {seed} / {mt}: N = {n_subj}")
        print(f"{'=' * 60}")

        if n_subj < 2:
            print(f"Need >= 2 subjects for permutation testing, skipping.")
            continue
        ...
        # Save outputs
        out_mean['t'].to_filename(out_dir / f"group_mean_{seed}_{mt}_tstat.nii.gz")
        out_mean['logp_max_tfce'].to_filename(out_dir / f"group_mean_{seed}_{mt}_logp_max_tfce.nii.gz")
        ...
        plotting.plot_stat_map(
            ...
            title=f"{seed} {mt} group mean (TFCE p<.05 FWE)",
            output_file=out_dir / f"group_mean_{seed}_{mt}.png",
        )
        print(f"  Saved group mean outputs for {seed}/{mt}")

        # -------------------------------------------------------
        # 2) Covariate analysis: left amygdala persistence
        # -------------------------------------------------------
        df = pd.DataFrame({'subject': subjects_with_maps[seed][mt],
                            'map_idx': range(len(subjects_with_maps[seed][mt]))})
        ...
        cov_maps = [maps[seed][mt][i] for i in df['map_idx']]
        ...
        out_cov['t'].to_filename(out_dir / f"group_covariate_{seed}_{mt}_tstat.nii.gz")
        out_cov['logp_max_tfce'].to_filename(out_dir / f"group_covariate_{seed}_{mt}_logp_max_tfce.nii.gz")

        plotting.plot_stat_map(
            out_cov['logp_max_tfce'], display_mode='z', cut_coords=7,
            threshold=1.3,
            title=f"{seed} {mt} persistence covariate (TFCE p<.05 FWE)",
            output_file=out_dir / f"group_covariate_{seed}_{mt}.png",
        )
        print(f"  Saved covariate outputs for {seed}/{mt}")
