...
# For each subject, extracts mean beta within each ROI for each condition.
#
# ROIs:
#   - L/R amygdala: Harvard-Oxford atlas (50% threshold, 2mm)
#   - Anterior vmPFC: 10mm sphere at [-2, 46, -10]
#   - Posterior vmPFC: 10mm sphere at [0, 26, -12]
#
# Conditions extracted:
#   neg_image, neu_image, pos_image, neg_face, neu_face, pos_face
#
# Output (one file per subject, then combined across subjects):
#   <out_dir>/<subid>/<subid>_roi_activations.csv
#     Columns: subid, run, <roi>_<condition> ...
#   Combined: <out_dir>/all_subjects_roi_activations.csv
#
...
print(f"Extracting ROI activations for {subid}")
...
# Spherical vmPFC ROIs (Tashjian et al., 2021)
ant_vmPFC_coords = [(-2, 46, -10)]   # anterior vmPFC
post_vmPFC_coords = [(0, 26, -12)]   # posterior vmPFC
...
# -------------------------------------------------------
# Conditions expected from runGLM.sh output
# -------------------------------------------------------
...
for run in runs:
    ...
    for cond in conditions:
        ...
        # Fit maskers on first available beta image (sets space)
