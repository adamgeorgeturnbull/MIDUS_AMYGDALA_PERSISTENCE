...
Identical to run_cross_corr_vmPFC.py but uses aCompCor GLM output
(24 motion + 6 aCompCor regressors) for preprocessing comparison.

Input:
    /scratch/groups/fvlin/MIDUS/M3/voxelwise_vmPFC_betas_aCompCor/<subid>/
        <subid>_voxelwise_vmPFC_betas.csv
    Columns: subject, run, condition, seed, nvox_resampled, beta_0..N

Output:
    /scratch/groups/fvlin/MIDUS/M3/vmPFC_persistence_aCompCor_summary/
        ...
        results_wide_vmPFC_persistence.csv     — wide format for merging with behavioral data
"""
...
# ========== USER SETTINGS ==========
BASE_DIR = Path("/scratch/groups/fvlin/MIDUS/M3/voxelwise_vmPFC_betas_aCompCor")
OUT_DIR  = Path("/scratch/groups/fvlin/MIDUS/M3/vmPFC_persistence_aCompCor_summary")
...
if df_summary.empty:
    print("ERROR: No data collected — check that extract_vmPFC_aCompCor.sh produced .csv files in:")
    print(f"  {BASE_DIR}/<subid>/<subid>_voxelwise_vmPFC_betas.csv")
    print("Inspect the extract_vmPFC_aCompCor SLURM log files for errors.")
    sys.exit(1)

# Wide format: one row per subject, columns = seed_condition_mean_r
