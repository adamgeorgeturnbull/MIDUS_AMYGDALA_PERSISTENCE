...
examining each condition (negative, neutral, positive) separately rather
than the neg > neu contrast. This diagnostic analysis addresses whether
null findings for the contrast (Analysis 05) reflect:
  (a) no amygdala-vmPFC connectivity-affect association at all, or
  (b) a real association that is present across conditions and therefore
...
Output:
  - results/tables/05b_fc_condition_correlations_conservative.csv
  - results/tables/05b_fc_condition_correlations_full.csv
...
def run_sample_analysis(df, sample_name, fc_vars, affect_vars):
    ...
    # Save
    out_file = RESULTS_DIR / f"05b_fc_condition_correlations_{sample_name}.csv"
...
def main():
    ...
    print("=" * 70)
    print("Analysis 05b: Condition-Level FC and Affect")
    print("=" * 70)
