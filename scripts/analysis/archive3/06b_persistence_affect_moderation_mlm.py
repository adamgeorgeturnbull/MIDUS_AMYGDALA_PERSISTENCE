...
Mixed-effects (multilevel) reanalysis of persistence x affect moderation
by emotion regulation strategy.

Replaces the OLS + twin-pair-dummy approach from 06_persistence_affect_moderation.py
with linear mixed-effects models (random intercept for family). This avoids the
...
Fixed effects: persistence_c + moderator_c + persistence_c*moderator_c +
               age + sex + race dummies + time_P2_P5 + n_days_complete
...
Outputs:
- results/tables/06b_persistence_affect_moderation_mlm_full.csv
- results/tables/06b_persistence_affect_moderation_mlm_conservative.csv
...
def run_mlm_moderation(df, persistence_var, affect_var, moderator, covariates):
    ...
    """
    vars_needed = [affect_var, persistence_var, moderator, "family_id"] + covariates
    ...
    persist_c = data[persistence_var] - data[persistence_var].mean()
    mod_c = data[moderator] - data[moderator].mean()
    interaction = persist_c * mod_c

    # Add centered variables to data for formula interface
    data = data.copy()
    persist_c_name = f"{persistence_var}_c"
    mod_c_name = f"{moderator}_c"
    interaction_name = f"{persistence_var}_x_{moderator}"
    data[persist_c_name] = persist_c
    ...
    predictors = [persist_c_name, mod_c_name, interaction_name] + active_covariates
...
def run_all_moderations(df, persistence_vars, affect_vars, moderators,
    ...
    """Run all persistence x affect x moderator mixed-effects models.
    ...
    for moderator in moderators:
        for persist_var in persistence_vars:
            for affect_var in affect_vars:
                ...
                result = run_mlm_moderation(df, persist_var, affect_var, moderator, covariates)
...
def run_sample_analysis(sample, sample_name, persistence_vars, affect_vars,
    ...
    out_file = RESULTS_DIR / f"06b_persistence_affect_moderation_mlm_{sample_name}.csv"
...
def main():
    ...
    print("=" * 80)
    print("Analysis 06b (MLM): Persistence x Affect -- Moderation by Emotion Regulation")
    print("=" * 80)
    ...
    print(f"Total models per sample: {len(persistence_vars)} x {len(affect_vars)} "
          f"x {len(moderators)} = {len(persistence_vars) * len(affect_vars) * len(moderators)}")
