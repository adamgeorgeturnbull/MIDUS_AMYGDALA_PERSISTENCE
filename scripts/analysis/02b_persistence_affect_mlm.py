...
def run_mlm(df, persistence_var, affect_var, covariates):
    ...
    with warnings.catch_warnings():
        ...
        se_p = result.bse_fe[persistence_var]
    degenerate_re = False
    if np.isnan(se_p):
        degenerate_re = True
        idx = list(result.fe_params.index).index(persistence_var)
        var = result.cov_params().iloc[idx, idx]
        se_p = np.sqrt(abs(var)) if abs(var) > 0 else np.nan
    z_p = result.fe_params[persistence_var] / se_p if not np.isnan(se_p) else np.nan
    if not np.isnan(z_p):
        from scipy import stats as _stats
        p_p = 2 * (1 - _stats.norm.cdf(abs(z_p)))
    else:
        p_p = np.nan
    ...
    return {
        ...
        "z_persistence": z_p,
        "p_persistence": p_p,
        "p_persistence_one_tailed": p_p / 2 if not np.isnan(p_p) else np.nan,
...
def run_sample_analysis(sample, sample_name, persistence_vars, affect_vars,
    ...
    if len(results) > 0:
        ...
        def _fmt_mlm(row):
            sig = ("***" if row["p_persistence"] < 0.001
                   else "**" if row["p_persistence"] < 0.01
        ...
        print_by_tier(results, _fmt_mlm, p_col="p_persistence")
    else:
        print("\n  No models computed (insufficient data)")

    # Save
    out_file = RESULTS_DIR / f"02b_persistence_affect_mlm_{sample_name}.csv"
...
def main():
    ...
    print("=" * 80)
    print("Analysis 02b (MLM): Persistence x Affect — Mixed-Effects Models")
    print("=" * 80)
