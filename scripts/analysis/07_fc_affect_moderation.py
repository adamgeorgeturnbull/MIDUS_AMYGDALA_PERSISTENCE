...
vmPFC functional connectivity (neg−neu contrast) and daily life affect?

Model (run separately for each moderator):
  affect ~ FC_c + moderator_c + FC_c × moderator_c + covariates

Both FC and moderator are mean-centered within each complete-case subset
before forming the interaction term.
...
Sensitivity analyses (PANAS, right amygdala) in 07_sensitivity.py.
...
# Configuration
# ============================================================================
OUT_DIR    = RESULTS_DIR / "07_fc_affect_moderation"
PREDICTORS = [
...
def run_moderation_set(df, predictors, outcomes, moderator, base_covs):
    ols_rows, mlm_rows = [], []
...
def main():
    print("=" * 70)
    print("Analysis 07: FC × ERQ → Daily Affect  (two-tailed)")
    print("=" * 70)
    ...
    for mod_var, mod_name in MODERATORS:
        ...
        ols_f, mlm_f = run_moderation_set(full, PREDICTORS, OUTCOMES, mod_var, base_covs_full)
        save_moderation(ols_f, mlm_f,
                        OUT_DIR / mod_name / "full_sample",
                        f"07 FC × {mod_name} → Daily Affect  [full sample, two-tailed]")
